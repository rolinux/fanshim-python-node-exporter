...
parser.add_argument('--prom_temp_file', type=str, default='/etc/node_exporter/fanshim_soc_temp.prom', help='Prom file with SoC temperature')
parser.add_argument('--prom_cpu_freq_file', type=str, default='/etc/node_exporter/fanshim_cpu_freq.prom', help='Prom file with cpu freq')
parser.add_argument('--prom_fan_enabled_file', type=str, default='/etc/node_exporter/fanshim_fan_enabled.prom', help='Prom file with fan status')
...
try:
    ...
    while True:
        t = get_cpu_temp()
        with open(args.prom_temp_file, 'w') as prom_temp:
           prom_temp.write('fanshim_soc_temperature {:05.02f}\n'.format(t))
        f = get_cpu_freq()
        with open(args.prom_cpu_freq_file, 'w') as prom_cpu_freq:
           prom_cpu_freq.write('fanshim_cpu_freq {:.0f}\n'.format(f.current))
        if args.verbose:
        ...
        if abs(last_change - t) > args.hysteresis and armed:
            ...
            if set_fan(enable):
                last_change = t
        with open(args.prom_fan_enabled_file, 'w') as prom_fan_enabled:
           prom_fan_enabled.write('fanshim_fan_enabled %d\n' % int(enabled))
